...
def train_model(ctx: click.Context, model_key: str, target_col: str, params: dict,
    ...
    if validation_index is not None:
        ...
        all_data.loc[validation_index, [f"{t}_pred" for t in target_col]] = model.predict(
...
def inference(ctx, numerai_model_name):
    ...
    live_data.loc[:, targets_pred] = model.predict(
    ...
    live_data.loc[:, ['target_pred_ensemble']] = ensembler_model.predict(
        live_data.loc[:, targets_pred])

    # neutralize
    print("Neutralizing predictions")
    live_data["half_neutral_target_pred_ensemble"] = neutralize(
        df=live_data,
        columns=["target_pred_ensemble"],
        neutralizers=features,
        proportion=ctx.obj['PARAMS']['neutralize_params']['proportion'],
    ...
    live_data["prediction"] = live_data['half_neutral_target_pred_ensemble'].rank(pct=True)
