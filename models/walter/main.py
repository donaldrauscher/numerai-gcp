...
def train_model(ctx: click.Context,
                model_class: Union[CatBoostRegressor, LGBMRegressor], model_key: str, params: dict,
                target_col: str, all_data: pd.DataFrame, training_index: pd.Index,
                validation_index: pd.Index = None, validation_pred_col: str = None):
    ...
    if model:
        ...
        if model_class == CatBoostRegressor:
            params = {**dict(objective='RMSEWithUncertainty', posterior_sampling=True), **params}
    ...
    if validation_index is not None:
        print(f"Adding predictions to {validation_pred_col} in validation data")
        if model_class == CatBoostRegressor:
            model_expected_features = model.feature_names_
            preds = model.predict(all_data.loc[validation_index, model_expected_features])[:,1]
        else:
            model_expected_features = model.booster_.feature_name()
            preds = model.predict(all_data.loc[validation_index, model_expected_features])
        all_data.loc[validation_index, validation_pred_col] = preds
...
def train(ctx):
    ...
    # final model config
    model_config = {
        "model_key_mean": ("train" if ctx.obj['TEST'] else "all_date") + '_mean',
...
def inference(ctx, numerai_model_name):
    ...
    model_mean = load_model(ctx, model_config['model_key_mean'])
    model_var = load_model(ctx, model_config['model_key_var'])

    model_expected_features = model_mean.booster_.feature_name()
