...
def train(ctx):
    ...
    targets = list(training_data.filter(like='target_').columns)
    ...
    target_training_index = all_data.loc[training_index, targets].dropna().index
    target_validation_index = all_data.loc[validation_index, targets].dropna().index
    target_all_index = all_data.loc[:, targets].dropna().index
    ...
    # train target vs. other targets to get a "smooth" target
    print("Creating blended target")
    targets2 = list(set(targets) - set([ctx.obj['PARAMS']['target_params']['target_col']]))

    target_model = Ridge(alpha=0.05)
    target_model.fit(
        all_data.loc[target_all_index, targets2],
        all_data.loc[target_all_index, ctx.obj['PARAMS']['target_params']['target_col']]
    )

    all_data.loc[target_all_index, ['target_blend_only']] = target_model.predict(
        all_data.loc[target_all_index, targets2])
    all_data['target_blend'] = all_data[['target_blend_only', ctx.obj['PARAMS']['target_params']['target_col']]].mean(axis=1, skipna=False)

    print("Target R^2 score: {}".format(r2_score(
        all_data[ctx.obj['PARAMS']['target_params']['target_col']][target_all_index],
        all_data['target_blend_only'][target_all_index]
    )))

    # neutralize target
    all_data["target_neutral"] = neutralize(
        df=all_data.loc[target_all_index, :],
        columns=['target_blend'],
